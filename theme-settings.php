...
function parabola_get_backgrounds() {
  $theme_name = 'parabola';
...
function parabola_get_backgrounds_bg() {
...
function parabola_form_system_theme_settings_alter(&$form, &$form_state, $form_id = NULL)  {
  global $base_url;

  // Work-around for a core bug affecting admin themes. See issue #943212.
  
  $current_theme_path = drupal_get_path('theme', 'parabola');
  // Load parabola Features
  ...
  if ($form['var']['#value'] == 'theme_parabola_settings') {
  ...
  $form['parabola_example'] = array(
    '#type'          => 'checkbox',
    '#title'         => t('parabola sample setting'),
    '#default_value' => theme_get_setting('parabola_example'),
 ...
 $form['at']['skins']['skins_type'] = array(
  ...
  foreach(parabola_get_backgrounds() as $bg) {
    $variables = array(
      'path' => $bg->uri,
      'alt' => $bg->name,
      'title' => $bg->name,
  ...
  foreach(parabola_get_backgrounds_bg() as $bg_image_temp) {
    $variables_bg = array(
